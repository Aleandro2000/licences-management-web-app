  ...
  async read (universityDto: UniversityDto): Promise<any> {
    try {
      if (universityDto.id) {
        ...
        if (university) { return { status: 200, result: university } } else { return { status: 400, message: 'Failed to read university details!' } }
      } else { return { status: 400, message: 'Failed to read university details!' } }
    } catch (err) {
      return { status: 400, message: err }
    }
  }
